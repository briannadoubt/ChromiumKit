...
@main
struct ChromiumKitIntegratePlugin: CommandPlugin {
    func performCommand(context: PluginContext, arguments: [String]) async throws {
        let projectURL = try resolveProjectURL(in: context.package.directoryURL)
        try runTool(named: "ChromiumKitCLI", arguments: ["integrate", "--project", projectURL.path], context: context)
    }
    ...
    private func resolveProjectURL(in directoryURL: URL) throws -> URL {
        ...
        guard let projectURL = projects.first else {
            throw PluginError("No Xcode project was found next to Package.swift. Run `swift run chromiumkit integrate --project /path/to/MyApp.xcodeproj` instead.")
        }
        guard projects.count == 1 else {
            let names = projects.map(\.lastPathComponent).joined(separator: ", ")
            throw PluginError("Multiple Xcode projects were found: \(names). Run `swift run chromiumkit integrate --project /path/to/YourApp.xcodeproj` instead.")
        }
        return projectURL
    }
}
...
#if canImport(XcodeProjectPlugin)
extension ChromiumKitIntegratePlugin: XcodeCommandPlugin {
    func performCommand(context: XcodePluginContext, arguments: [String]) throws {
        try runTool(
            named: "ChromiumKitCLI",
            arguments: ["integrate", "--project", context.xcodeProject.directoryURL.path],
            context: context
        )
    }
}
...
private func runTool(named toolName: String, arguments: [String], context: some Any) throws {
    ...
    if process.terminationStatus != 0 {
        throw PluginError("ChromiumKit integration failed.")
    }
}
